...
Единая точка входа в документацию платформы. Здесь — цель проекта, финальное видение, архитектура, реестр сервисов, roadmap, статус-дашборд и глоссарий. Подробная спека каждого этапа — в [docs/stages/](stages/). Компактная витрина этапов и сабмодулей (Mermaid) — [project-status.md](project-status.md).
